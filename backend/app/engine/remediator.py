...
def generate_remediation_recipes(
    ...
    # 1. Class Imbalance Recipe (Severe or Moderate)
    ...
    # 4. Target Leakage Prune Recipe
    leakage_cols = risk_flags.get("leakage_columns", [])
    if leakage_cols:
        recipes.append({
            "title": "Remove Suspected Leaky Features",
            "description": f"Features {leakage_cols} have near-perfect correlation (>= 0.98) with target. Remove them.",
            "code": (
    ...
    # 5. Default Clean Baseline Pipeline
    if not recipes:
        recipes.append({
